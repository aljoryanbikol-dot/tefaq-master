...
import { Mic, MicOff, Square, Play, ArrowRight, Clock, Loader2, CheckCircle, AlertCircle, TrendingUp } from "lucide-react";
...
export default function SpeakingPage() {
  ...
  const startRecording = useCallback(async () => {
    if (user?.plan === "free") {
      toast.error("L'evaluation orale complete est reservee aux membres Premium.");
      return;
    }
    try {
  ...
  const evaluateRecording = async () => {
    ...
    try {
      ...
      const mockEval: SpeakingEvaluation = {
        overall_score: 72,
        cefr_level: user?.current_level || "B1",
    ...
    return (
      <AppLayout title="Expression orale">
        <div className="space-y-6">
          ...
          {user?.plan === "free" && (
            <Card className="border-accent-200 dark:border-accent-800 bg-gradient-to-r from-accent-50 to-primary-50 dark:from-accent-950/20 dark:to-primary-950/20">
              <CardBody className="p-4 flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-accent-500 flex items-center justify-center flex-shrink-0"><Mic size={20} className="text-white" /></div>
                <div className="flex-1">
                  <p className="font-semibold text-surface-900 dark:text-white text-sm">Fonctionnalite Premium</p>
                  <p className="text-xs text-surface-500 mt-0.5">L'evaluation orale complete avec IA est disponible avec le plan Premium.</p>
                </div>
                <Button size="sm" className="bg-accent-500 hover:bg-accent-600 flex-shrink-0">Passer a Premium</Button>
              </CardBody>
            </Card>
          )}
        ...
        <div className="max-w-2xl mx-auto space-y-6 animate-slide-up">
          ...
          <div className="flex gap-3">
            ...
            <Button className="flex-1" size="lg" icon={<Mic size={18} />} onClick={startRecording}
              ...
              {user?.plan === "free" ? "Premium requis" : "Commencer l'enregistrement"}
            </Button>
          </div>
        </div>
      </AppLayout>
